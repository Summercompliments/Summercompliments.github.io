...
app.post('/api/submit', upload.single('photo'), async (req, res) => {
  try {
    ...
    form.append('photo', req.file.buffer, { filename: 'entry.jpg', contentType: req.file.mimetype || 'image/jpeg' });
    ...
    if (!tgRes.data.ok) {
      throw new Error(tgRes.data.description || 'Telegram rejected the request');
    }
    res.json({ ok: true });
  } catch (err) {
    const tgError = err.response && err.response.data && err.response.data.description;
    console.error('Submit failed:', tgError || err.message);
    res.status(500).json({ ok: false, error: tgError || err.message });
  }
});
